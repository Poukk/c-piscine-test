...
void cpt_test_exercise() {
    print_exercise("02", "ft_iterative_power");
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        snprintf(actual, sizeof(actual), "%d", ft_iterative_power(nb, power));
        
        char test_name[50];
        snprintf(test_name, sizeof(test_name), "Test %d: ft_iterative_power(%d, %d)", i + 1, nb, power);
